...
cmd="""export CUDA_VISIBLE_DEVICES=$CUR_NODE && export NCCL_DEBUG=INFO && export NCCL_SOCKET_IFNAME=ens5 && export GLOO_SOCKET_IFNAME=ens5 && \
    ...
    ./scripts/run-project-pactum-slave.sh $NUM_NODES $NUM_STAGES $GLOBAL_RANK $MICRO_BATCH_SIZE $SEQ_LEN $LAYERS' > res/lab_aws/nodes_${NUM_NODES}_${NUM_STAGES}_${MICRO_BATCH_SIZE}/node_$CUR_NODE.txt 2>&1"""
